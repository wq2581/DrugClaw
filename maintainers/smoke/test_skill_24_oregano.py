...
FIXTURE = ROOT / "resources_metadata" / "drug_repurposing" / "OREGANO" / "oregano.csv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "evidence" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing evidence")
    return problems


def main() -> int:
    from skills.drug_repurposing.oregano import OREGANOSkill

    checks = [
        "import OREGANOSkill",
        f"instantiate OREGANOSkill(csv_path={FIXTURE})",
        "call is_available()",
    ...
    skill = OREGANOSkill(config={"csv_path": str(FIXTURE)})
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for a fixture-backed OREGANO skill.")
    ...
    if problems:
        fixes.append("Keep OREGANOSkill aligned with the local-file contract used by maintainers/smoke/test_skill_24_oregano.py.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_24_oregano.py"],
