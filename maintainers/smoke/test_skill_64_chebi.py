#!/usr/bin/env python3
"""
Real smoke test for skills.drug_ontology.chebi.ChEBISkill.
"""
...
def validate_results(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        if not isinstance(metadata, dict) or not metadata:
            ...
            if "chebi_id" not in metadata:
                problems.append(f"{label}: result[{i}] metadata missing chebi_id")
    return problems


def main() -> int:
    commands = ["python maintainers/smoke/test_skill_64_chebi.py"]
    checks = [
        "read chebi_skill.py",
        "read README.md",
        "read SKILL.md",
    ...
    from skills.drug_ontology.chebi import ChEBISkill

    skill = ChEBISkill(config={"timeout": 20})
    sample["is_available"] = skill.is_available()
    if not sample["is_available"]:
        problems.append("is_available() returned False for an implemented ontology skill.")

    r1 = skill.retrieve({"drug": ["aspirin"]}, query="ontology", max_results=5)
    r2 = skill.retrieve({"drug": ["imatinib"]}, query="ontology", max_results=5)
    edge = skill.retrieve({"drug": ["zzz_not_a_real_drug_zzz"]}, query="ontology", max_results=5)
    ...
    if problems:
        fixes.append("Keep the runtime REST fallback aligned with ChEBI 2.0 public API; the older search.do endpoint is no longer valid.")
