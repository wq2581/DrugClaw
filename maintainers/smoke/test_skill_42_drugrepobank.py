...
FIXTURE = ROOT / "resources_metadata" / "drug_repurposing" / "DrugRepoBank" / "drugrepobank.csv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        if not isinstance(meta, dict) or "status" not in meta:
            problems.append(f"{label}: result[{i}] metadata missing status")
    return problems


def main() -> int:
    from skills.drug_repurposing.drugrepobank import DrugRepoBankSkill
    checks = ["import DrugRepoBankSkill", f"instantiate DrugRepoBankSkill(csv_path={FIXTURE})", "call is_available()", "standard query: drug=imatinib", "edge query: drug=zzz_not_a_real_drug_zzz", "validate evidence_text and metadata"]
    skill = DrugRepoBankSkill(config={"csv_path": str(FIXTURE)})
    ...
    problems = ([] if sample["is_available"] else ["is_available() returned False for a fixture-backed DrugRepoBank skill."]) + validate(standard, "standard_query")
    if edge: problems.append("edge_query: expected empty list for non-existent drug")
    status = "PASS" if not problems else "FAIL"
    fixes = [] if not problems else ["Keep DrugRepoBankSkill aligned with the local-file contract used by maintainers/smoke/test_skill_42_drugrepobank.py."]
    print(json.dumps(asdict(Report(status, ["python maintainers/smoke/test_skill_42_drugrepobank.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
