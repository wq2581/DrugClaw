...
FIXTURE = ROOT / "resources_metadata" / "drug_nlp" / "CADEC" / "cadec.csv"
...
def main() -> int:
    from skills.drug_nlp.cadec import CADECSkill
    checks = ["import CADECSkill", f"instantiate CADECSkill(csv_path={FIXTURE})", "call is_available()", "standard query: drug=lipitor", "edge query: drug=zzz_not_a_real_drug_zzz", "validate evidence_text and metadata"]
    skill = CADECSkill(config={"csv_path": str(FIXTURE)})
    sample = {"is_available": skill.is_available()}
    standard = skill.retrieve({"drug": ["lipitor"]}, query="social media ade", max_results=5)
    edge = skill.retrieve({"drug": ["zzz_not_a_real_drug_zzz"]}, query="social media ade", max_results=5)
    sample["result_count"] = len(standard); sample["edge_count"] = len(edge); sample["sample_results"] = [rr_view(r) for r in standard[:2]]
    problems = ([] if sample["is_available"] else ["is_available() returned False for a fixture-backed CADEC skill."]) + validate(standard, "standard_query")
    if edge: problems.append("edge_query: expected empty list for non-existent drug")
    status = "PASS" if not problems else "FAIL"
    fixes = [] if not problems else ["Keep CADECSkill aligned with the local-file contract used by maintainers/smoke/test_skill_34_cadec.py."]
    print(json.dumps(asdict(Report(status, ["python maintainers/smoke/test_skill_34_cadec.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
