...
FIXTURE = ROOT / "resources_metadata" / "drug_toxicity" / "UniTox" / "unitox.csv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "organ_system" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing organ_system")
    return problems


def main() -> int:
    from skills.drug_toxicity.unitox import UniToxSkill

    checks = [
        "import UniToxSkill",
        f"instantiate UniToxSkill(csv_path={FIXTURE})",
        "call is_available()",
    ...
    skill = UniToxSkill(config={"csv_path": str(FIXTURE)})
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for a fixture-backed UniTox skill.")
    ...
    if problems:
        fixes.append("Keep UniToxSkill aligned with the local-file contract used by maintainers/smoke/test_skill_28_unitox.py.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_28_unitox.py"],
