...
FIXTURE = ROOT / "resources_metadata" / "drug_combination" / "DrugCombDB" / "drugcombdb.csv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "synergy_score" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing synergy_score")
    return problems


def main() -> int:
    from skills.drug_combination.drugcombdb import DrugCombDBSkill

    checks = [
        "import DrugCombDBSkill",
        f"instantiate DrugCombDBSkill(csv_path={FIXTURE})",
        "call is_available()",
    ...
    skill = DrugCombDBSkill(config={"csv_path": str(FIXTURE)})
    sample["is_available"] = skill.is_available()
    standard = skill.retrieve({"drug": ["imatinib"]}, query="combination", max_results=5)
    edge = skill.retrieve({"drug": ["zzz_not_a_real_drug_zzz"]}, query="combination", max_results=5)
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for a fixture-backed DrugCombDB skill.")
    ...
    if problems:
        fixes.append("Keep DrugCombDBSkill aligned with the local-file contract used by maintainers/smoke/test_skill_21_drugcombdb.py.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_21_drugcombdb.py"],
