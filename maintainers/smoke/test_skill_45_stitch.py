...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "stitch_score" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing stitch_score")
    return problems


def main() -> int:
    from skills.dti.stitch import STITCHSkill
    ...
    skill = STITCHSkill(config={"timeout": 20})
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for an implemented STITCH skill.")
    ...
    if problems:
        fixes.append("Keep STITCHSkill aligned with the example script and current offline/live fallback contract.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_45_stitch.py"],
