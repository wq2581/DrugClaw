...
FIXTURE = ROOT / "resources_metadata" / "drug_repurposing" / "RepurposeDrugs" / "repurposedrugs.csv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        if not isinstance(meta, dict) or "score" not in meta:
            problems.append(f"{label}: result[{i}] metadata missing score")
    return problems


def main() -> int:
    from skills.drug_repurposing.repurposedrugs import RepurposeDrugsSkill
    checks = ["import RepurposeDrugsSkill", f"instantiate RepurposeDrugsSkill(csv_path={FIXTURE})", "call is_available()", "standard query: drug=imatinib", "edge query: drug=zzz_not_a_real_drug_zzz", "validate evidence_text and metadata"]
    skill = RepurposeDrugsSkill(config={"csv_path": str(FIXTURE)})
    ...
    problems = ([] if sample["is_available"] else ["is_available() returned False for a fixture-backed RepurposeDrugs skill."]) + validate(standard, "standard_query")
    if edge: problems.append("edge_query: expected empty list for non-existent drug")
    status = "PASS" if not problems else "FAIL"
    fixes = [] if not problems else ["Keep RepurposeDrugsSkill aligned with the local-file contract used by maintainers/smoke/test_skill_43_repurposedrugs.py."]
    print(json.dumps(asdict(Report(status, ["python maintainers/smoke/test_skill_43_repurposedrugs.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
