...
FIXTURE = ROOT / "resources_metadata" / "drug_nlp" / "n2c2_2018" / "n2c2_2018.tsv"
...
def main() -> int:
    from skills.drug_nlp.n2c2_2018 import N2C22018Skill
    checks = ["import N2C22018Skill", f"instantiate N2C22018Skill(tsv_path={FIXTURE})", "call is_available()", "standard query: drug=warfarin", "edge query: drug=zzz_not_a_real_drug_zzz", "validate evidence_text and metadata"]
    skill = N2C22018Skill(config={"tsv_path": str(FIXTURE)})
    sample = {"is_available": skill.is_available()}
    standard = skill.retrieve({"drug": ["warfarin"]}, query="ehr ade", max_results=5)
    edge = skill.retrieve({"drug": ["zzz_not_a_real_drug_zzz"]}, query="ehr ade", max_results=5)
    sample["result_count"] = len(standard); sample["edge_count"] = len(edge); sample["sample_results"] = [rr_view(r) for r in standard[:2]]
    problems = ([] if sample["is_available"] else ["is_available() returned False for a fixture-backed n2c2 2018 skill."]) + validate(standard, "standard_query")
    if edge: problems.append("edge_query: expected empty list for non-existent drug")
    status = "PASS" if not problems else "FAIL"
    fixes = [] if not problems else ["Keep N2C22018Skill aligned with the local-file contract used by maintainers/smoke/test_skill_33_n2c2_2018.py."]
    print(json.dumps(asdict(Report(status, ["python maintainers/smoke/test_skill_33_n2c2_2018.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
