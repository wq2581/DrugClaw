...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "severity" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing severity")
    return problems


def main() -> int:
    from skills.ddi.ddinter import DDInterSkill
    ...
    skill = DDInterSkill(config={"timeout": 20})
    sample["is_available"] = skill.is_available()
    standard = skill.retrieve({"drug": ["aspirin"]}, query="interaction", max_results=5)
    edge = skill.retrieve({"drug": ["zzz_not_a_real_drug_zzz"]}, query="interaction", max_results=5)
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for an implemented DDI skill.")
    ...
    if problems:
        fixes.append("Keep DDInterSkill aligned with the example script and current offline/live fallback contract.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_20_ddinter.py"],
