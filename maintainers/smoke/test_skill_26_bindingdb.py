...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "uniprot_id" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing uniprot_id")
    return problems


def main() -> int:
    from skills.dti.bindingdb import BindingDBSkill
    ...
    skill = BindingDBSkill(config={"timeout": 20})
    sample["is_available"] = skill.is_available()
    standard = skill.retrieve({"drug": ["imatinib"]}, query="binding", max_results=5)
    edge = skill.retrieve({"drug": ["zzz_not_a_real_drug_zzz"]}, query="binding", max_results=5)
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for an implemented BindingDB skill.")
    ...
    if problems:
        fixes.append("Keep BindingDBSkill aligned with the example script and current offline/live fallback contract.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_26_bindingdb.py"],
