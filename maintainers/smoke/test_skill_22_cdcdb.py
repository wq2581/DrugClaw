...
FIXTURE = ROOT / "resources_metadata" / "drug_combination" / "CDCDB" / "cdcdb.csv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "cancer_type" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing cancer_type")
    return problems


def main() -> int:
    from skills.drug_combination.cdcdb import CDCDBSkill

    checks = [
        "import CDCDBSkill",
        f"instantiate CDCDBSkill(csv_path={FIXTURE})",
        "call is_available()",
    ...
    skill = CDCDBSkill(config={"csv_path": str(FIXTURE)})
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for a fixture-backed CDCDB skill.")
    ...
    if problems:
        fixes.append("Keep CDCDBSkill aligned with the local-file contract used by maintainers/smoke/test_skill_22_cdcdb.py.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_22_cdcdb.py"],
