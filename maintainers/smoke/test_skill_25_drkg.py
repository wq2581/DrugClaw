...
FIXTURE = ROOT / "resources_metadata" / "drug_repurposing" / "DRKG" / "drkg.tsv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        elif "raw_relation" not in metadata:
            problems.append(f"{label}: result[{i}] metadata missing raw_relation")
    return problems


def main() -> int:
    from skills.drug_repurposing.drkg import DRKGSkill

    checks = [
        "import DRKGSkill",
        f"instantiate DRKGSkill(drkg_tsv={FIXTURE})",
        "call is_available()",
    ...
    skill = DRKGSkill(config={"drkg_tsv": str(FIXTURE)})
    sample["is_available"] = skill.is_available()
    standard = skill.retrieve({"drug": ["imatinib"]}, query="knowledge graph", max_results=5)
    edge = skill.retrieve({"drug": ["zzz_not_a_real_drug_zzz"]}, query="knowledge graph", max_results=5)
    ...
    if not sample["is_available"]:
        problems.append("is_available() returned False for a fixture-backed DRKG skill.")
    ...
    if problems:
        fixes.append("Keep DRKGSkill aligned with the local-file contract used by maintainers/smoke/test_skill_25_drkg.py.")

    print(json.dumps(asdict(Report(
        status=status,
        commands_executed=["python maintainers/smoke/test_skill_25_drkg.py"],
