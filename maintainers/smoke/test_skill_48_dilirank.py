...
FIXTURE = ROOT / "resources_metadata" / "drug_toxicity" / "DILIrank" / "dilirank.csv"
...
def validate(results: list[Any], label: str) -> list[str]:
    ...
    for i, item in enumerate(results):
        ...
        if not isinstance(meta, dict) or "rank" not in meta:
            problems.append(f"{label}: result[{i}] metadata missing rank")
    return problems


def main() -> int:
    from skills.drug_toxicity.dilirank import DILIrankSkill
    checks = ["import DILIrankSkill", f"instantiate DILIrankSkill(csv_path={FIXTURE})", "call is_available()", "standard query: drug=acetaminophen", "edge query: drug=zzz_not_a_real_drug_zzz", "validate evidence_text and metadata"]
    skill = DILIrankSkill(config={"csv_path": str(FIXTURE)})
    ...
    problems = ([] if sample["is_available"] else ["is_available() returned False for a fixture-backed DILIrank skill."]) + validate(standard, "standard_query")
    if edge: problems.append("edge_query: expected empty list for non-existent drug")
    status = "PASS" if not problems else "FAIL"
    fixes = [] if not problems else ["Keep DILIrankSkill aligned with the local-file contract used by maintainers/smoke/test_skill_48_dilirank.py."]
    print(json.dumps(asdict(Report(status, ["python maintainers/smoke/test_skill_48_dilirank.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
