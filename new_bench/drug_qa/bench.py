"""
Drug QA — PubMedQA filtered benchmark.

Task  : Given a biomedical question + background context, answer yes / no / maybe.
Labels: yes, no, maybe
Data  : new_bench_data/PubMedQA/pubmedqa_pqa_labeled_train_drug_samples.json
        JSON array [{pubid, question, context, long_answer, final_decision}, ...]
        context is a dict (or dict-like string) with keys:
            contexts  — list of background paragraph strings
            labels    — sentence-level labels
Metric: Accuracy (exact match on yes/no/maybe)
"""

import ast
import json
from pathlib import Path

DATA_PATH = (
    Path(__file__).resolve().parent.parent.parent
    / "new_bench_data/PubMedQA/pubmedqa_pqa_labeled_train_drug_samples.json"
)

LABELS = ["yes", "no", "maybe"]

SYSTEM_PROMPT = """\
You are a biomedical question-answering assistant.

Task: Read the question and the provided background context, then answer with
      one of exactly three words: yes, no, or maybe.

Rules:
- "yes"   if the evidence in the context clearly supports an affirmative answer.
- "no"    if the evidence clearly does not support it.
- "maybe" if the evidence is insufficient, mixed, or uncertain.

Respond with ONLY a JSON object: {"answer": "<label>"}
where <label> is one of: yes, no, maybe
"""

USER_TEMPLATE = """\
Question: {question}

Background context:
{context}

Answer (yes / no / maybe):
"""


def _parse_context(raw) -> str:
    """Extract background paragraphs from the context field."""
    if isinstance(raw, dict):
        ctx = raw
    else:
        try:
            ctx = ast.literal_eval(str(raw))
        except Exception:
            return str(raw)[:1000]

    paragraphs = ctx.get("contexts", [])
    if paragraphs:
        return "\n\n".join(str(p) for p in paragraphs[:5])
    return str(raw)[:1000]


def load_data(max_samples: int = 0) -> list[dict]:
    with open(DATA_PATH, encoding="utf-8") as f:
        raw = json.load(f)

    rows = []
    for r in raw:
        gold = r.get("final_decision", "").strip().lower()
        if gold not in LABELS:
            continue
        rows.append({
            "question": r["question"],
            "context": _parse_context(r["context"]),
            "gold": gold,
        })

    if max_samples > 0:
        rows = rows[:max_samples]
    return rows


def run(
    key_file: str | None = None,
    max_samples: int = 0,
    mode: str = "direct",
    maskself: bool = False,
    log_dir: str | None = None,
) -> dict:
    from new_bench.bench_utils import run_bench

    return run_bench(
        dataset_name="drug_qa",
        labels=LABELS,
        default_label="maybe",
        system_prompt=SYSTEM_PROMPT,
        samples=load_data(max_samples),
        format_prompt=lambda s: USER_TEMPLATE.format(
            question=s["question"],
            context=s["context"],
        ),
        mode=mode,
        maskself=maskself,
        key_file=key_file,
        log_dir=log_dir,
    )
