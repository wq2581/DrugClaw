#!/usr/bin/env python3
"""
Fixed retrieval CLI for PHEE (Pharmacovigilance Events from EHR).
...
from example import load_phee, search_batch, summarize
...
def main():
    ...
    try:
        records = load_phee()
        results = search_batch(records, entities)
        for entity, hits in results.items():
            print(summarize(hits, entity))
    except Exception as exc:
        print(f"[PHEE] Error: {exc}", file=sys.stderr)
        sys.exit(1)
