...
**Purpose**: Drug target commons  

**Coverage**: Community-curated drug-target bioactivity database  

## Setup

1. Download the dataset from: <https://drugtargetcommons.fimm.fi/static/Excell_files/DTC_data.csv>
2. Set `csv_path` (or `tsv_path` / `json_path`) in config (see below).

## Usage

```python
from drugclaw.skills.dti.dtc import DTCSkill

skill = DTCSkill(config={
...
- Homepage: <https://drugtargetcommons.fimm.fi/static/Excell_files/DTC_data.csv>
