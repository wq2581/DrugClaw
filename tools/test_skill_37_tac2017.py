...
def main() -> int:
    ...
    fixes = [] if not problems else ["Keep TAC2017ADRSkill aligned with the local-file contract used by tools/test_skill_37_tac2017.py."]
    print(json.dumps(asdict(Report(status, ["python tools/test_skill_37_tac2017.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
