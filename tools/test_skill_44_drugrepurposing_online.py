...
def main() -> int:
    ...
    fixes = [] if not problems else ["Keep DrugRepurposingOnlineSkill aligned with the local-file contract used by tools/test_skill_44_drugrepurposing_online.py."]
    print(json.dumps(asdict(Report(status, ["python tools/test_skill_44_drugrepurposing_online.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
