...
def main() -> int:
    ...
    fixes = [] if not problems else ["Keep PsyTARSkill aligned with the local-file contract used by tools/test_skill_36_psytar.py."]
    print(json.dumps(asdict(Report(status, ["python tools/test_skill_36_psytar.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
