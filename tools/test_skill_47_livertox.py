...
def main() -> int:
    ...
    fixes = [] if not problems else ["Keep LiverToxSkill aligned with the offline/live fallback contract used by tools/test_skill_47_livertox.py."]
    print(json.dumps(asdict(Report(status, ["python tools/test_skill_47_livertox.py"], checks, sample, problems, fixes)), indent=2, ensure_ascii=False))
